...
/* 매개 변수 (parameter, argument)
	 ...
	 1.매개변수는 메서드를 호출할 때,
	 메서드 실행에 필요한 값들을 메서드 내부로 전달하는 매개체 역할을 합니다.
	  
	 2.메서드 호출부에서 어떤 매개값을 전달하느냐에 따라
	 메서드의 실행 결과는 달라질 수 있습니다.
	 
	 3.매개값을 몇 개 받아서 사용할지는 메서드를 선언할 때,
	 선언부에서 () 미리 개수와 타입을 지정합니다.
	 
	 4.매개변수를 하나도 선언하지 않을 수도 있습니다.
	 이때는 메서드의 선언부에 () 괄호 내부를 비워둡니다.
	 그리고 호출할 때도 () 괄호 내부를 비워서 호출합니다.
...
public class MethodExample2 {
	...
	public static void main(String[] args) {
		...
		sum = calcNumberTotal(new int[] {1,2,3,4,5,6,7}); //(일회성)1~7까지의 합이 궁금하다 하면 이렇게 배열 짜면 됩니다.
		System.out.println("누적합: " + sum);

...
/*
		 ...
		 반환값은 오직 한 개만 존재해야 합니다.
		 
		 3.전달되는 값의 타입을 반환 유형이라고 하며,
		 메서드를 선언할 때 메서드 이름 앞에 반드시 반환 유형을 명시해야 합니다.
		 
		 4.return 이라는 키워드를 사용하며 return 뒤에 전달할 값을 지정합니다.
		 (변수, 상수)
		 
		 5.반환값이 있는 메서드는 호출문 자체가 하나의 값으로 처리될 수 있기 때문에
		 반환값을 다른 변수에 대입할 수도 있고, 다른 메서드의 매개값으로도 처리될 수 있습니다.
		 
		 6.모든 메서드가 항상 반환값이 있는 것은 아닙니다.
		 메서드 실행 후에 반환할 값이 딱히 없다면 return을 생략해도 됩니다.
		 
		 7.이때는 반환유형을 메서드 선언할 때 비워두지 마시고,
...
public class MethodExample3 {
	...
	static void divide(int n1, int n2) {
		if(n2 ==0) {
			System.out.println("0으로 나누지 마세요 -_-");
			return;
		}
		int result = n1 / n2;
	...
	public static void main(String[] args) {
		...
		int[] resusult = operateTotal(7, 6);
		System.out.println("7+6=" + resusult[0]);
		System.out.println("7-6=" + resusult[1]);
		System.out.println("7*6=" + resusult[2]);
		System.out.println("7/6=" + resusult[3]);
		...
		/*
		 ...
		 실행 결과를 변수에 저장할 수 없고, 다른 메서드의 매개값으로도 사용이 불가능합니다.
...
//		add(multi(4,4), multi(5,6)); (x) multi를 다시 가져오지 못함
		...
		/*
		 ...
		 모든 정수의 합계(int)와 평균(double)을 '배열'에 담아서 리턴하는 메서드입니다.
		 3. 2번에 정의한 조건대로 메서드를 선언하시고, 실제로 main에서 호출해서
		 합계와 평균(소수 2까지)을 출력해 주세요.
		 {57,89,78,91,93,47}

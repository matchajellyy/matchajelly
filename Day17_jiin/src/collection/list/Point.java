...
public class Point {
	...
	public void inputPoints(List<Student> students) {
		...
		students.add(stu);
		System.out.printf("%s 님의 성적 정보가 정상적으로 입력되었습니다.\n"
				, stu.getName());
	...
	public void showAllPoints(List<Student> students) {
		...
		showPointUI();
		for(Student stu : students) { // students 객체 주소가 stu로 들어옵니다.
			stu.outputStuInfo();
			totalAvg += stu.getAvg();		   
		}

		System.out.println("=-===================");
		System.out.printf("\t\t\t\t\t우리반 전체 평균: %.2점\n",
				totalAvg / students.size());
	}

	//3. 개별 성적 조회 로직을 처리할 메서드
	public void searchPoint(List<Student> students) {
		System.out.println("성적을 조회할 학생의 학번을 입력해 주세요.");
		System.out.print("> ");
		...
		for(Student stu : students) {
			if(stuNum.equals(stu.getStuId())){
				...
				while(true) {
					try {
						System.out.print("국어: ");
						stu.setKor(sc.nextInt()); //set한테 보내서 바로 저장하겠다.
						System.out.print("영어: ");
	...
	public void deletePoint(List<Student> students) {
		...
		System.out.println("삭제할 학생의 학번을 입력해 주세요.");
		System.out.print("> ");
		String stuNum = sc.next();
		...
		for(Student stu : students) {
		if(stuNum.equals(stu.getStuId())){
			System.out.printf("%s님의 성적 정보를 삭제할까요? [Y/N]\n"
					, stu.getName());
			System.out.println("> ");
